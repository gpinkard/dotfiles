#+STARTUP: overview
#+TITLE: Gabe's Emacs Configuration
#+CREATOR: Gabriel Pinkard
* Cookies
#+BEGIN_SRC emacs-lisp
  ;; -*- lexical-binding: t; -*-
#+END_SRC
* Custom Variables
#+BEGIN_SRC emacs-lisp
  ;; Default font size to use for most faces.
  (setq gabe/font-size 120)

  ;; Default font to use for a number of faces.
  (setq gabe/default-font "Hack")

  ;; Holds state on whether a demap buffer exists or not. This is uesed in
  ;; gabe/split-*-and-follow functions to not mess up the rebalancing of windows.
  (setq gabe/demap-is-open nil)

  ;; Whether or not to enable hl-line-mode for a variety of modes.
  (setq gabe/enable-hl-line-mode t)

  ;; Whether or not to enable line numbers for a variety of modes.
  (setq gabe/enable-line-numbers t)
#+END_SRC
* User Interface 
User interface configurations (i.e. tool bar, cursor, etc.).
** tool bar
Disable the tool bar.
#+BEGIN_SRC emacs-lisp
  (tool-bar-mode 0)
#+END_SRC
** menu bar
Disable the menubar.
#+BEGIN_SRC emacs-lisp
  (menu-bar-mode 0)
#+END_SRC
** scroll bar
Disable the scroll bar.
#+BEGIN_SRC emacs-lisp
  (scroll-bar-mode 0)
#+END_SRC
** cursor
Blinking cursor configurations.
#+BEGIN_SRC emacs-lisp
  ;; disable blinking cursor
  (blink-cursor-mode 0)

  ;; blink indefinitely when blinking enabled
  (setq blink-cursor-blinks 0)

  ;; time to start blinking when blinking enabled
  (setq blink-cursor-delay 0.3)
#+END_SRC
Set cursor shape.
#+BEGIN_SRC emacs-lisp
  (setq-default cursor-type 'box)
#+END_SRC
** startup screen
Disable the startup screen.
#+BEGIN_SRC emacs-lisp
  (setq inhibit-startup-message t)
#+END_SRC
** dialog prompts
Stop emacs from using UI dialog boxes.
#+BEGIN_SRC emacs-lisp
  (setq use-dialog-box nil)
#+END_SRC
** time
Display current time in the modeline.
#+BEGIN_SRC emacs-lisp
  (display-time-mode 1)
#+END_SRC
** echo area
Configurations for the echo area.
#+BEGIN_SRC emacs-lisp
  ;; disable the "You can run the command {x} with keybinding {y}" messages
  (setq suggest-key-bindings nil)
#+END_SRC
* Editing
Configurations that impact the editing experience and general feel of emacs.
** parens/braces/quotes/etc.
Show matching parens, quotes, braces, etc.
#+BEGIN_SRC emacs-lisp
  (show-paren-mode t)
#+END_SRC
Paren visual match delay.
#+BEGIN_SRC emacs-lisp
  (setq blink-matching-delay 0.5)
#+END_SRC
Complete braces, quotes, parens, etc.
#+BEGIN_SRC emacs-lisp
  (electric-pair-mode t)
#+END_SRC
** file encoding
Set default coding system to UTF-8.
#+BEGIN_SRC emacs-lisp
  (set-default-coding-systems 'utf-8)
#+END_SRC
** tabs/offset
Configurations for tab/offset size for various languages.
#+BEGIN_SRC emacs-lisp
  ;; C indentation level
  (add-hook 'c-mode-hook
  	  (lambda ()
  	    (setq-local c-basic-offset 8)))

  ;; C++ indentation level
  (add-hook 'cpp-mode-hook
  	  (lambda ()
  	    (setq-local c-basic-offset 8)))

  ;; Java indentation level
  (add-hook 'java-mode-hook
  	  (lambda ()
  	    (setq-local c-basic-offset 4)))

  ;; JavaScript/TypeScript
  (dolist (hook '(js-mode-hook
  		js-ts-mode-hook
  		typescript-ts-mode-hook
  		typescript-mode-hook))
    (add-hook hook (lambda ()
  		   (setq-local js-indent-level 2)
  		   (setq-local typescript-ts-indent-offset 2)
  		   (setq-local tab-width 2))))

  ;; Odin indentation level
  (add-hook 'odin-ts-mode-hook
  	  (lambda ()
  	    (setq-local indent-tabs-mode t)
  	    (setq-local tab-width 4)))
#+END_SRC
** line numbers
Enable line numbers when we are in a programming mode.
#+BEGIN_SRC emacs-lisp
  (when (and (version<= "26.0.50" emacs-version) gabe/enable-line-numbers)
    (add-hook 'markdown-mode-hook #'display-line-numbers-mode)
    (add-hook 'prog-mode-hook #'display-line-numbers-mode)
    (add-hook 'toml-mode-hook #'display-line-numbers-mode)
    (add-hook 'yaml-mode-hook #'display-line-numbers-mode))
#+END_SRC
** line wrap
Wrap long lines visually.
#+BEGIN_SRC emacs-lisp
  (global-visual-line-mode t)
#+END_SRC
** remember place in file
Have emacs remember the last cursor position of a file you visited.
#+BEGIN_SRC emacs-lisp
  (save-place-mode t)
#+END_SRC
** automatically refresh buffers
Automatically refresh buffers on file changes (i.e. git checkout).
#+BEGIN_SRC emacs-lisp
  (global-auto-revert-mode 1)

  ;; log that file was reloaded to *Messages* buffer
  (setq auto-revert-verbose t)
#+END_SRC
** scrolling
Configurations that effect how emacs scrolls text.
#+BEGIN_SRC emacs-lisp
  ;; scroll line by line instead of half a buffer at a time
  (setq scroll-conservatively 100)
#+END_SRC
** alias yes-or-no-p
Alias 'yes' and 'no' to 'y' and 'n' for all emacs confirmation flows.
#+BEGIN_SRC emacs-lisp
  (defalias 'yes-or-no-p 'y-or-n-p)
#+END_SRC
** backups/autosaving
Disable the creation of backup files/autosaving.
#+BEGIN_SRC emacs-lisp
  ;; no backup files
  (setq make-backup-files nil)

  ;; no auto save files
  (setq auto-save-default nil)
#+END_SRC
** disable audio bell
Disable the audio beep for keyboard-quit
#+BEGIN_SRC emacs-lisp
  (setq ring-bell-function 'ignore)
#+END_SRC
** hightlight line mode
Modes to enable hl-line-mode in.
#+BEGIN_SRC emacs-lisp
  (when gabe/enable-hl-line-mode
    (add-hook 'dired-mode-hook #'hl-line-mode)
    (add-hook 'ibuffer-hook #'hl-line-mode)
    (add-hook 'markdown-mode-hook #'hl-line-mode)
    (add-hook 'toml-mode-hook #'hl-line-mode)
    (add-hook 'org-mode-hook #'hl-line-mode)
    (add-hook 'package-menu-mode-hook #'hl-line-mode)
    (add-hook 'prog-mode-hook #'hl-line-mode)
    (add-hook 'yaml-mode-hook #'hl-line-mode))
#+END_SRC
* Dired
Settings for directory editor.
#+BEGIN_SRC emacs-lisp
  ;; Make dired refresh directory contents on changes.
  (setq global-auto-revert-non-file-buffers t)
#+END_SRC
* OSX
This configuration targets gnu/linux, but I often have to use a mac for work. These
configurations make using emacs for osx more bareable.
#+BEGIN_SRC emacs-lisp
  (setq mac-command-modifier 'meta)
#+END_SRC
* Org
Configurations for org mode.
** source code editing
Force org to edit src blocks in the same window.
#+BEGIN_SRC emacs-lisp
  (setq org-src-window-setup 'other-window)
#+END_SRC
* Browser/EWW
Configurations for browsers in emacs, and the emacs web wowser.
#+BEGIN_SRC emacs-lisp
  ;; set default browser to eww
  (setq browse-url-browser-function 'eww-browse-url)

  ;; use duckduckgo by default
  (setq eww-search-prefix "https://duckduckgo.com/html?q=")
#+END_SRC
* Terminal
Configurations for the built in terminals in emacs.
** ansi-term
Force ansi-term to use zsh.
#+BEGIN_SRC emacs-lisp
  (defvar term-shell "/bin/zsh")
  (defadvice ansi-term (before force-zsh)
    (interactive (list term-shell)))
  (ad-activate 'ansi-term)
#+END_SRC
Global keybinding to open a terminal in the current frame.
#+BEGIN_SRC emacs-lisp
  ;; (global-set-key (kbd "C-c <return>") 'ansi-term)
#+END_SRC
* Ibuffer
Bind C-x C-b to ibuffer instead of list-buffers.
#+BEGIN_SRC emacs-lisp
  (global-set-key (kbd "C-x C-b") 'ibuffer)
#+END_SRC
Disable confirmation messages when killing a buffer in ibuffer.
#+BEGIN_SRC emacs-lisp
  (setq ibuffer-expert t)
#+END_SRC
* My Functions
Custom functions I have written for various tasks.
** demap is open
Returns true if a demap buffer is open
#+BEGIN_SRC emacs-lisp
  ;; Returns true if there is a demap buffer open, else nil.
  (defun gabe/demap-buffer-is-open-p ()
    (let ((minimap-buffer (get-buffer "*Minimap*")))
      (and minimap-buffer
	   (if (get-buffer-window minimap-buffer) t nil))))
#+END_SRC
** split and follow windows
Functions for following a window after split.
#+BEGIN_SRC emacs-lisp
  ;; Better horizontal split.
  (defun gabe/split-horizontally-and-follow ()
    (interactive)
    (setq gabe/demap-is-open (gabe/demap-buffer-is-open-p))
    (if gabe/demap-is-open (demap-toggle))
    (split-window-below)
    (balance-windows)
    (other-window 1)
    (if gabe/demap-is-open (demap-toggle)))

  (global-set-key (kbd "C-x 2") 'gabe/split-horizontally-and-follow)

  ;; Better vertical split.
  (defun gabe/split-vertically-and-follow ()
    (interactive)
    (setq gabe/demap-is-open (gabe/demap-buffer-is-open-p))
    (if gabe/demap-is-open (demap-toggle))
    (split-window-right)
    (balance-windows)
    (other-window 1)
    (if gabe/demap-is-open (demap-toggle)))

  (global-set-key (kbd "C-x 3") 'gabe/split-vertically-and-follow)
#+END_SRC
** reload config
Reload the configuration file.
#+BEGIN_SRC emacs-lisp
  (defun gabe/reload-config ()
    (interactive)
    (message "reloading config...")
    (org-babel-load-file (expand-file-name "~/.emacs.d/config.org"))
    (message "done..."))

  (global-set-key (kbd "C-c c r") 'gabe/reload-config)
#+END_SRC
** open config
Open the configuration file in the current frame.
#+BEGIN_SRC emacs-lisp
  (defun gabe/open-config ()
    (interactive)
    (find-file "~/.emacs.d/config.org"))

  (global-set-key (kbd "C-c c e") 'gabe/open-config)
#+END_SRC
** load custom faces
Sets custom face values using the set-face-attribute function.
#+BEGIN_SRC emacs-lisp
  (defun gabe/load-custom-face-attributes ()
    (interactive)
    ;; make ace-window face larger
    (custom-set-faces '(aw-leading-char-face ((t (:inherit ace-jump-face-foreground :height 5.0)))))
    
    (set-face-attribute 'default nil
    		      :font gabe/default-font
    		      :height gabe/font-size
    		      :weight 'regular
    		      :width 'regular)

    (set-face-attribute 'fixed-pitch nil
    		      :font gabe/default-font
    		      :height gabe/font-size
    		      :weight 'regular
    		      :width 'regular)

    (set-face-attribute 'fixed-pitch-serif nil
    		      :font gabe/default-font
    		      :height gabe/font-size
    		      :weight 'regular
    		      :width 'regular)

    (set-face-attribute 'variable-pitch nil
    		      :font gabe/default-font
    		      :height gabe/font-size
    		      :weight 'regular
    		      :width 'regular))
#+END_SRC
** set font size
Interactive function which sets the gabe/font-size variable to.
set the font size globaly.
#+BEGIN_SRC emacs-lisp
  (defun gabe/set-font-size (size)
    (interactive "nFont Size: ")
    (setq gabe/font-size size)
    (gabe/load-custom-face-attributes))

  (global-set-key (kbd "C-c c f") 'gabe/set-font-size)
#+END_SRC
* Packages
** package archives
Setup the package archives to use.
#+BEGIN_SRC emacs-lisp
  (require 'package)

  (setq package-archives '(("melpa" . "https://melpa.org/packages/")
  			   ;;("melpa-stable" . "https://stable.melpa.org/packages/")
  			   ("org" . "https://orgmode.org/elpa/")
  			   ("elpa" . "https://elpa.gnu.org/packages/")))

  (package-initialize)

  ;; (setq package-install-upgrade-built-in t)
#+END_SRC
** use package
Package that is a macro for installing and configuring packages.
#+BEGIN_SRC emacs-lisp
  (unless (package-installed-p 'use-package)
    (package-refresh-contents)
    (package-install 'use-package))

  (eval-when-compile
    (require 'use-package))
#+END_SRC
** icons
#+BEGIN_SRC emacs-lisp
  (use-package nerd-icons
    :ensure t)
    ;; :config
    ;; (setq nerd-icons-font-family "RobotoMono Nerd Font Mono"))

  (use-package nerd-icons-dired
    :ensure t
    :hook
    (dired-mode . nerd-icons-dired-mode))

  (use-package nerd-icons-ibuffer
    :ensure t
    :hook
    (ibuffer-mode . nerd-icons-ibuffer-mode)
    :config
    (setq nerd-icons-ibuffer-icon t)
    (setq nerd-icons-ibuffer-human-readable-size t)
    (setq nerd-ibuffer-color-icon t))

  (use-package nerd-icons-completion
    :ensure t
    :after marginalia
    :config
    (nerd-icons-completion-mode)
    :hook
    (marginalia-mode . nerd-icons-completion-marginalia-setup))
#+END_SRC
** vertico
Vertical completion framework that works nicely with native emacs completions.
#+BEGIN_SRC emacs-lisp
  (use-package vertico
    :ensure t
    :init
    (vertico-mode)
    (vertico-indexed-mode)
    (vertico-mouse-mode)
    :custom
    (vertico-count 20)
    (vertico-cycle t)
    (vertico-resize t))
#+END_SRC
** savehist
Saves minibuffer history to an external file. Vertico uses this to sort the position
of completions.
#+BEGIN_SRC emacs-lisp
  (use-package savehist
    :ensure t
    :init
    (savehist-mode))
#+END_SRC
** marginalia
Adds useful information to minibuffer completions.
#+BEGIN_SRC emacs-lisp
  (use-package marginalia
    :after vertico
    :ensure t
    :init
    (marginalia-mode))
#+END_SRC
** orderless
A better (orderless) completion style.
#+BEGIN_SRC emacs-lisp
  (use-package orderless
    :ensure t
    :custom
    (completion-styles '(orderless basic))
    (completion-category-overrides '((file (styles basic partial-completion)))))
#+END_SRC
** consult
Better search and navigation commands (similar to Counsel package).
#+BEGIN_SRC emacs-lisp
  (use-package consult
    :ensure t
    :hook
    (completion-list-mode . consult-preview-at-point-mode)
    :init
    (setq register-preview-delay 0.5)
    (setq register-preview-function #'consult-register-format)
    :bind
    ("C-s" . consult-line)
    ("C-x b" . consult-buffer)
    ("M-g M-g" . consult-goto-line)
    ("M-y" . consult-yank-pop)
    ("C-c p c g" . affe-grep)
    ("C-c p c f" . affe-find))

  ;; git version control information in consult
  (use-package consult-vc-modified-files
    :ensure t
    :bind
    ("C-c p c v" . consult-vc-modified-files))
#+END_SRC
** consult symbol
Search accross all emacs symbols (functions, variables, macros, etc.) using consult.
#+BEGIN_SRC emacs-lisp
  (use-package consult-symbol
    :ensure t
    :bind
    ("C-c p c s" . consult-symbol))
#+END_SRC
** affe
Asynchronous Fuzzy Finder for Emacs.
#+BEGIN_SRC emacs-lisp
  (use-package affe
    :ensure t)

  (defun affe-orderless-regexp-compiler (input _type _ignorecase)
    (setq input (cdr (orderless-compile input)))
    (cons input (apply-partially #'orderless--highlight input t)))

  (setq affe-regexp-compiler #'affe-orderless-regexp-compiler)
#+END_SRC
** projectile
Package for easily finding files and switching between projects.
#+BEGIN_SRC emacs-lisp
  (use-package projectile
    :ensure t
    :init
    (projectile-mode +1)
    :config
    ;; (setq projectile-indexing-method 'alien)
    (setq projectile-sort-order 'recentf)
    (setq projectile-enable-caching t)
    (setq projectile-project-search-path '("~/dotfiles" "~/Projects" "~/.emacs.d"))
    :bind
    ("C-c p p" . projectile-command-map))
#+END_SRC
** avy
A package that allows you to jump to text quickly.
#+BEGIN_SRC emacs-lisp
  (use-package avy
    :ensure t
    :bind
    ("C-;" . avy-goto-char-2))
#+END_SRC
** repeat mode
Repeat commands easily.
#+BEGIN_SRC emacs-lisp
  (use-package repeat
    :ensure t
    :config
    (repeat-mode 1))
#+END_SRC
** multiple cursors
Multiple cursors for editing multiple lines at once.
#+BEGIN_SRC emacs-lisp
  (use-package multiple-cursors
    :ensure t
    :bind
    ("C-c p m e" . mc/edit-lines)
    ("C-c p m l a" . mc/mark-all-like-this)
    ("C-c p m l n" . mc/mark-next-like-this)
    ("C-c p m l p" . mc/mark-previous-like-this)
    ("C-c p m w a" . mc/mark-all-like-this-word)
    ("C-c p m w n" . mc/mark-all-next-like-this-word)
    ("C-c p m w p" . mc/mark-all-previous-like-this-word))
#+END_SRC
** magit
A *magical* git porcelain.
#+BEGIN_SRC emacs-lisp
  (use-package magit
    :ensure t)
#+END_SRC
** ace window
Better window switching (as well as other window options).
#+BEGIN_SRC emacs-lisp
  (use-package ace-window
    :ensure t
    :config
    (setq aw-keys '(?a ?s ?d ?f ?g ?h ?j ?k ?l))
    :bind
    ("C-x o" . ace-window))
#+END_SRC
** which key
Package that shows completions of key-chords in a minibuffer.
#+BEGIN_SRC emacs-lisp
  (use-package which-key
    :ensure t
    :init
    (which-key-mode)
    :config
    (setq which-key-compute-remaps t)
    (setq which-key-show-docstrings t)
    (setq which-key-idle-delay 1.5))
#+END_SRC
** language specific modes
Programming language modes.
#+BEGIN_SRC emacs-lisp
  ;; clojure
  (use-package clojure-mode
    :ensure t)

  ;; crystal
  (use-package crystal-mode
    :ensure t)

  ;; docker
  (use-package dockerfile-mode
    :ensure t)

  ;; golang
  (use-package go-ts-mode
    :ensure t)

  ;; graphql
  (use-package graphql-mode
    :ensure t)

  ;; markdown
  (use-package markdown-mode
    :ensure t)

  ;; odin
  (use-package odin-ts-mode
    :vc
    (:url "https://github.com/Sampie159/odin-ts-mode")
    :ensure t
    :config
    (add-to-list 'auto-mode-alist '("\\.odin\\'" . odin-ts-mode)))

  ;; python
  (use-package python-mode
    :ensure t)

  ;; ruby
  (use-package ruby-ts-mode
    :ensure t)
  (use-package ruby-end
    :ensure t
    :hook
    (ruby-mode . ruby-end-mode))

  ;; rust
  (use-package rust-mode
    :ensure t)

  ;; toml
  (use-package toml-mode
    :ensure t)

  ;; typescript
  (use-package typescript-ts-mode
    :ensure t)

  ;; yaml
  (use-package yaml-mode
    :ensure t)

  ;; zig
  (use-package zig-ts-mode
    :ensure t)

  ;; treesitter grammars
  ;; after editing run M-x treesit-install-language-grammar RET {language} RET
  (setq treesit-language-source-alist
        '((go "https://github.com/tree-sitter/tree-sitter-go")
  	(odin "https://github.com/tree-sitter-grammars/tree-sitter-odin")
  	(rust "https://github.com/tree-sitter/tree-sitter-rust")
  	(tsx "https://github.com/tree-sitter/tree-sitter-typescript" "master" "tsx/src")
  	(typescript "https://github.com/tree-sitter/tree-sitter-typescript" "master" "typescript/src")
  	(zig "https://github.com/tree-sitter/zig-tree-sitter")))
#+END_SRC
** flymake
#+BEGIN_SRC emacs-lisp
  (use-package flymake
    :ensure t
    :hook
    (prog-mode . flymake-mode)
    :bind
    ("C-c f n" . flymake-goto-next-error)
    ("C-c f p" . flymake-goto-prev-error)
    ("C-c f d" . flymake-show-buffer-diagnostics))
#+END_SRC
** eglot
Emacs Polyglot (LSP).
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    :ensure t
    :hook
    ((rust-mode typescript-ts-mode typescript-mode js-ts-mode elisp-mode) . eglot-ensure)
    (eglot-managed-mode . (lambda () (add-hook 'before-save-hook #'eglot-format-buffer nil t)))
    :config
    (with-eval-after-load 'eglot
      (add-to-list 'eglot-server-programs
  		 '(rust-mode . ("rust-analyzer"
  				:initializationOptions (:procMacro (:enable t)
  								   :cargo (:buildScripts (:enable t) :features "all")))))
      (add-to-list 'eglot-server-programs
  		 '(typescript-ts-mode . ("typescript-language-server" "--stdio")))

      (add-to-list 'eglot-server-programs
    		 '(js-ts-mode . ("typescript-language-server" "--stdio"))))
    (advice-add 'eglot-rename :after (lambda (&rest _) (projectile-save-project-buffers)))
    (setq eglot-autoshutdown t)
    (setq eglot-sync-connect nil)
    (setq eglot-report-progress t)
    :bind
    ("C-c e r" . eglot-rename)
    ("C-c e f" . eglot-format)
    ("C-c e c" . eglot-code-actions)
    ("C-c e o" . eglot-code-actions-organize-imports)
    ("C-c e h s" . eglot-show-type-hierarchy)
    ("C-c e h t" . eglot-call-type-hierarchy))
#+END_SRC
** eldoc box
Display eldoc messages in posframe box.
#+BEGIN_SRC emacs-lisp
  (use-package eldoc-box
    :ensure t
    :after eglot
    :hook
    (eglot-managed-mode . eldoc-box-hover-mode)
    :config
    (setq eldoc-box-fringe-use-same-bg nil))
#+END_SRC
** dape
Debugger adapter protocol package (Debugger Adapter Protocol for Emacs).
#+BEGIN_SRC emacs-lisp
  (use-package dape
    :ensure t
    :hook
    (kill-emacs . dape-breakpoint-save)
    (after-init . dape-breakpoint-load)
    :config
    (add-hook 'dape-start-hook (lambda () (save-some-buffers t t))))
#+END_SRC
** company mode
COMPlete ANYthing text completion framework.
#+BEGIN_SRC emacs-lisp
  (use-package company
    :ensure t
    :hook
    (prog-mode . company-mode))
#+END_SRC
** vterm
A faster, more responsive terminal in emacs. May need to install whatever the libvterm package
is on the OS.
#+BEGIN_SRC emacs-lisp
  ;; vterm package itself
  (use-package vterm
    :ensure t
    :config
    (setq vterm-always-compile-module t)
    (setq vterm-term-environment-variable "eterm-color"))
#+END_SRC
** multi vterm
Enables spawning multiple vterm instances instead of
jumping to an existing vterm buffer.
#+BEGIN_SRC emacs-lisp
  (use-package multi-vterm
    :ensure t
    :bind
    ("C-c <return>" . multi-vterm))
#+END_SRC
** vterm editor
Edit shell commands in a buffer and send to vterm.
#+BEGIN_SRC emacs-lisp
  (use-package vterm-editor
    :ensure t
    :bind
    (:map vterm-mode-map
  	("C-c e" . vterm-editor-open)))
#+END_SRC
** popterm
Toggle a terminal in a posframe.
#+BEGIN_SRC emacs-lisp
  (use-package popterm
    :ensure t
    :bind
    ("C-c SPC" . popterm-toggle)
    :config
    (setq popterm-backend 'vterm)
    (setq popterm-display-method 'posframe)
    (setq popterm-scope 'project)
    (setq popterm-auto-cd nil)
    (popterm-global-mode 1))
#+END_SRC
** demap (minimap)
Minimap showing file on the side of the buffer (like sublime).
#+BEGIN_SRC emacs-lisp
  (use-package demap
    :ensure t
    :bind
    ("C-c p d" . demap-toggle)
    :config
    (setq demap-minimap-window-side 'left)
    (setq demap-minimap-window-width 18))
#+END_SRC
** exec from shell
Make emacs use environment variables from the shell.
#+BEGIN_SRC emacs-lisp
  (use-package exec-path-from-shell
    :ensure t
    :init
    (when (memq window-system '(mac ns x))
      (exec-path-from-shell-initialize)))
#+END_SRC
** rainbow mode
Package that sets background of hex color codes the the color they represent.
#+BEGIN_SRC emacs-lisp
  (use-package rainbow-mode
    :ensure t
    :hook
    (prog-mode . rainbow-mode))
#+END_SRC
** denote
Sane note taking package.
#+BEGIN_SRC emacs-lisp
  (use-package denote
    :ensure t
    :hook
    (dired-mode . denote-dired-mode)
    :bind
    ("C-c p n n" . denote)
    ("C-c p n r" . denote-rename-file)
    ("C-c p n l" . denote-link)
    ("C-c p n d" . denote-dired)
    ("C-c p n g" . denote-grep)
    :config
    (setq denote-directory (expand-file-name "~/notes"))
    (setq denote-known-keywords '("work" "personal" "programming"))
    (setq denote-file-type 'org)
    (setq denote-infer-keywords t)
    (setq denote-sort-keywords t)
    (setq denote-rename-buffer-mode 1))
#+END_SRC
** elfeed
RSS feed reader for emacs.
#+BEGIN_SRC emacs-lisp
  (use-package elfeed
    :ensure t
    :bind
    ("C-c p e" . elfeed)
    :config
    (setq elfeed-use-curl t)
    (setq elfeed-db-directory "~/.emacs.d/elfeed")
    (setq elfeed-search-filter "@4-months-ago +unread")
    (setq elfeed-show-truncate-long-urls t)
    (setq elfeed-feeds
  	    '(("https://xkcd.com/atom.xml" comics)
  	      ("https://www.smbc-comics.com/comic/rss" comics)
  	      ("https://planet.emacslife.com/atom.xml" emacs)
  	      ("https://static.fsf.org/fsforg/rss/blogs.xml" fsf)
  	      ("https://static.fsf.org/fsforg/rss/news.xml" fsf)
  	      ("https://protesilaos.com/news.xml" prot news)
  	      ("https://protesilaos.com/politics.xml" prot politics)
  	      ("https://protesilaos.com/codelog.xml" prot emacs code)
  	      ("https://www.nasa.gov/rss/dyn/breaking_news.rss" space nasa)
  	      ("https://www.nasa.gov/rss/dyn/lg_image_of_the_day.rss" space nasa)
  	      ("http://blogs.nasa.gov/stationreport/feed/" space nasa iss)
  	      ("http://www.nasa.gov/rss/dyn/chandra_images.rss" space nasa chandra_mission)
  	      ("https://www.nasa.gov/rss/dyn/mission_pages/kepler/news/kepler-newsandfeatures-RSS.rss" space nasa keplar_mission)
  	      ("https://www.phoronix.com/rss.php" linux news)
  	      ("https://www.archlinux.org/feeds/news/" linux arch)
  	      ("https://www.linuxfoundation.org/feed/" linux))))

#+END_SRC
** hackernews
Hacker news client
#+BEGIN_SRC emacs-lisp
  (use-package hackernews-modern
    :ensure t
    :bind
    ("C-c p h" . hackernews-modern))
#+END_SRC
** dashboard
Emacs start screen replacement.
#+BEGIN_SRC emacs-lisp
  (use-package dashboard
    :ensure t
    :config
    (dashboard-setup-startup-hook)
    (setq dashboard-banner-logo-title "=== Gabe's Editor Macros ===")
    (setq dashboard-items '((projects . 5)
  			  (recents . 5)))
    (setq dashboard-set-navigator t)
    (setq dashboard-display-icons-p t)
    (setq dashboard-icon-type 'nerd-icons)
    (setq dashboard-set-heading-icons nil)
    (setq dashboard-set-file-icons t)
    (setq dashboard-footer-messages '("If a man knows not to which port he sails, no wind is favorable."
  				    "Pain is the touchstone of all growth."
  				    "What one programmer can do in one month, two programmers can do in two months."
  				    "Code is read much more often than it is written."
  				    "I may not be totally perfect, but parts of me are excellent."
  				    "Nothing is ever a total loss; it can always serve as a bad example."
  				    "Life is really simple, but we insist on making it complicated."
  				    "God drives the shortbus."
  				    "Indecision with the passing of time becomes decision."
  				    "Keep cool, but don't freeze."
  				    "Easy does it."
  				    "Nothing great is created suddenly."))
    (setq dashboard-startup-banner "~/.emacs.d/assets/hobbes.png"))
#+END_SRC
** spacious padding
Better padding around buffers.
#+BEGIN_SRC emacs-lisp
  (use-package spacious-padding
    :ensure t
    :bind
    ("C-c p s" . spacious-padding-mode)
    :config
    (setq spacious-padding-subtle-mode-line t))
    ;; :init
    ;; (spacious-padding-mode))
#+END_SRC
** life calendar
#+BEGIN_SRC emacs-lisp
  (use-package life-calendar
    :ensure t
    :config
    (setq life-calendar-birthday "1996-11-12")
    (setq life-calendar-years 95)
    (setq life-calendar-week-start-day 'birthday))
#+END_SRC
** dad jokes
What it says on the tin.
#+BEGIN_SRC emacs-lisp
  (use-package dad-joke
    :ensure t
    :bind
    ("C-c p j" . dad-joke))
#+END_SRC
** chess
#+BEGIN_SRC emacs-lisp
  (use-package chess
    :ensure t)
#+END_SRC
** theme
Color theme to use.
#+BEGIN_SRC emacs-lisp
  (use-package doric-themes
    :ensure t)

  (load-theme 'doric-borage t)
#+END_SRC
* Faces
Load faces by calling gabe/load-custom-face-attributes.
#+BEGIN_SRC emacs-lisp
  (gabe/load-custom-face-attributes)
#+END_SRC
