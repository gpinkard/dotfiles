...
iconinit(void)
{
	...
	if(tagcols[BACK] == nil) {
		...
		tagcols[TEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, COL_TEXT);
		tagcols[HTEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, COL_HTEXT);


		/* Text colors */
		textcols[BACK] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, TXT_BG);
		textcols[HIGH] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, TXT_HBG);
		textcols[BORD] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, TXT_BORD);
		textcols[TEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, TXT_TEXT);
		textcols[HTEXT] = allocimage(display, Rect(0,0,1,1), screen->chan, 1, TXT_HTEXT);
	}
	...
	button = allocimage(display, r, screen->chan, 0, TXT_BORD);
	draw(button, r, tagcols[BACK], nil, r.min);
	r.max.x -= ButtonBorder;
	border(button, r, ButtonBorder, tagcols[BORD], ZP);

	r = button->r;
	modbutton = allocimage(display, r, screen->chan, 0, COL_BORD);
	draw(modbutton, r, tagcols[BACK], nil, r.min);
	r.max.x -= ButtonBorder;
	border(modbutton, r, ButtonBorder, tagcols[BORD], ZP);
	r = insetrect(r, ButtonBorder);
	tmp = allocimage(display, Rect(0,0,1,1), screen->chan, 1, TXT_TEXT);
	draw(modbutton, r, tmp, nil, ZP);
	freeimage(tmp);

	r = button->r;
	colbutton = allocimage(display, r, screen->chan, 0, COL_BORD);
